...
\title{Tìm hiểu và xây dựng môi trường học tăng cường SAC cho robot UR5e thực hiện tác vụ gắp thả trong PyBullet}
...
\begin{abstract}
Bài báo trình bày chi tiết về thiết kế môi trường học tăng cường cho robot UR5e thực hiện tác vụ gắp và thả vật thể trong mô phỏng vật lý PyBullet. Tác vụ gắp thả liên tục được mô hình hóa dưới dạng một quy trình quyết định Markov với không gian quan sát 20 chiều và không gian hành động liên tục 7 chiều. Thuật toán Soft Actor Critic được lựa chọn để huấn luyện chính sách điều khiển cho robot từ đầu mà không cần qua giai đoạn tiền huấn luyện. Nhằm giải quyết các thách thức về sự thưa thớt của phần thưởng và hiện tượng tối ưu hóa sai lệch, chúng tôi đề xuất một cơ chế tay gắp lai tự động kích hoạt dựa trên khoảng cách vật lý và một bộ ràng buộc tư thế đầu hút kết hợp với hàm phần thưởng phân chia theo pha cụ thể. Hệ thống được huấn luyện trong 10 triệu bước thời gian với cấu trúc vector hóa song song 16 môi trường và áp dụng chuẩn hóa VecNormalize. Trong tập đánh giá thực nghiệm gồm 100 chu kỳ độc lập, mô hình không ghi nhận episode thất bại, với điểm phần thưởng trung bình đạt 569.4 và số bước thực thi trung bình là 26.3 bước cho mỗi chu kỳ. Các kết quả này cho thấy tính khả thi của phương pháp thiết kế MDP và hàm thưởng trong việc định hướng học tập cho tác tử robot liên tục. Cuối cùng, bài báo thảo luận về các giới hạn hiện tại liên quan đến khoảng cách giữa mô phỏng và thực tế.
\end{abstract}
...
Trong bài báo này, chúng tôi tập trung vào việc mô hình hóa và thiết kế chi tiết môi trường học tăng cường cho robot UR5e 6 bậc tự do trong phần mềm mô phỏng PyBullet. Chúng tôi đề xuất một cơ chế tay gắp lai tự động giúp đơn giản hóa không gian hành động của mạng neural, đồng thời áp dụng ràng buộc góc quay đầu hút để giữ cho giác hút luôn hướng xuống song song với mặt bàn. Hàm phần thưởng được thiết kế dựa trên ba pha hoạt động rõ rệt bao gồm: tiếp cận vật thể, nâng vật thể lên cao và di chuyển thả vật thể vào thùng chứa. Hệ thống được đánh giá chi tiết qua các đường cong học tập và thử nghiệm thực tế trong mô phỏng để ghi nhận tính ổn định trước khi thảo luận về các hướng phát triển trong tương lai.
...
Để đảm bảo tính chính xác của bộ mã nguồn động học tự thiết kế trong thư mục \texttt{kinematics}, chúng tôi đã xây dựng hai kịch bản kiểm chứng tự động độc lập:
\begin{itemize}
...
\subsection{Quy luật sinh vật thể và chương trình huấn luyện tăng dần}
Để nâng cao độ khó của nhiệm vụ và giúp chính sách học được các hành vi xoay khớp linh hoạt, chúng tôi áp dụng chương trình học tăng dần (Curriculum Learning) dựa trên ba mức độ khó khác nhau khi reset môi trường:
\begin{itemize}
...
Không gian quan sát của tác tử được thiết kế dưới dạng một vector liên tục 20 chiều, chứa đầy đủ các thông tin vị trí toàn cục, vị trí tương đối và thông số hướng của hệ thống robot vật thể. Chi tiết cấu trúc không gian quan sát được mô tả trong Bảng~\ref{tab:observation} \cite{gymnasiumspaces}.
...
\begin{itemize}
    ...
    \item Thành phần thứ bảy $a_{gripper}$ đại diện cho lệnh đóng mở tay gắp. Tuy nhiên, trong thiết kế đề xuất của chúng tôi, giá trị hành động này không được sử dụng điều khiển tay gắp mà được thay thế bằng cơ chế tay gắp lai tự động nhằm ổn định quá trình học.
\end{itemize}
...
\end{itemize}
Nhờ cơ chế này, mạng neural của tác tử không cần phải tập trung vào việc tối ưu hóa thời điểm đóng mở tay gắp mà dành dung lượng mạng cho việc tối ưu hóa quỹ đạo chuyển động trong không gian 3 chiều.
...
\begin{itemize}
    ...
    \end{equation}
    Khi điều kiện hoàn thành tác vụ được thỏa mãn (vật nằm gọn trong thùng và cao độ thấp dưới mức giới hạn), tác tử nhận điểm thưởng thành công cuối cùng rất lớn là $+500.0$ điểm và episode kết thúc.
...
Cần chú ý rằng toàn bộ chính sách được huấn luyện trong nghiên cứu này là từ đầu (train from scratch) trên tệp \texttt{train\_17d\_place.py} trong tổng số 10 triệu bước mẫu. Chúng tôi không sử dụng các kỹ thuật huấn luyện chuyển tiếp hay nạp trước trọng số từ các giai đoạn huấn luyện gắp trước đó, cho thấy hàm phần thưởng thiết kế hỗ trợ tốt để tác tử tự tìm ra chuỗi hành vi phức tạp từ đầu.
...
Hiệu suất ghi nhận trong môi trường mô phỏng cho thấy việc kết hợp các cơ chế thiết kế phụ trợ là yếu tố quan trọng hỗ trợ cho sự thành công của học tăng cường trong các tác vụ robot mô phỏng:
\begin{itemize}
    ...
    \item \textit{Ràng buộc hướng}: Ràng buộc góc cổ tay $\pm15^\circ$ giữ vai trò định hướng hình học cho giác hút hướng thẳng đứng tiếp cận vật, đồng thời giúp giới hạn nghiệm giải tích của bộ giải động học nghịch nằm trong vùng an toàn.
...
Mặc dù hệ thống không ghi nhận episode thất bại trong môi trường thử nghiệm, chúng tôi cần thảo luận các giới hạn biên của kết quả này để tránh các khẳng định quá mức:
\begin{itemize}
    \item \textit{Giới hạn mô phỏng}: Khả năng hoạt động tốt chỉ được ghi nhận trong phạm vi môi trường mô phỏng PyBullet dưới cấu hình phân phối vật thể mức khó trung bình (\texttt{difficulty=1}). Hệ thống chưa được kiểm chứng hiệu suất trên môi trường khó hơn (\texttt{difficulty=2}) hoặc với các chướng ngại vật di động.
    \item \textit{Thông tin trạng thái trong mô phỏng}: Tác tử hiện tại nhận thông tin tọa độ vị trí và hướng của vật thể được đọc trực tiếp từ lõi mô phỏng PyBullet. Trên thực tế, hệ thống robot thực tế không có được thông tin này mà phải thông qua các cảm biến camera đo chiều sâu hoặc thuật toán xử lý ảnh để ước lượng tư thế vật thể, điều này có thể đưa thêm sai số vào hệ thống.
    \item \textit{Chưa có robot vật lý thật}: Nghiên cứu này được thực hiện trên môi trường mô phỏng vật lý ảo. Chúng tôi chưa triển khai kiểm chứng chính sách điều khiển trên một cánh tay robot UR5e vật lý thật cũng như chưa thực hiện các nghiên cứu để giải quyết khoảng cách giữa mô phỏng và thực tế (sim to real gap) như kỹ thuật Domain Randomization hay học thích ứng trên hệ thật \cite{tobin2017domain}.
\end{itemize}

\section{Kết luận và Hướng phát triển}
Nghiên cứu này đã trình bày chi tiết phương pháp thiết kế và xây dựng môi trường học tăng cường sử dụng thuật toán SAC cho robot UR5e thực hiện tác vụ gắp thả vật thể trong mô phỏng PyBullet. Chúng tôi đã công thức hóa thành công bài toán dưới dạng quy trình quyết định Markov với quan sát 20 chiều và hành động Cartesian vi phân 7 chiều. Bằng việc đề xuất cơ chế tay gắp lai tự động hóa logic hút, bộ ràng buộc hướng đầu gắp chống xoắn khớp và hàm phần thưởng định hướng phân chia theo 3 pha hoạt động cụ thể, chính sách điều khiển của tác tử đã hội tụ nhanh chóng chỉ sau 600,000 bước huấn luyện từ đầu. Đánh giá thực nghiệm ghi nhận mô hình không gặp episode thất bại trong mô phỏng với chu kỳ thực thi trung bình dưới 27 bước thời gian. 
