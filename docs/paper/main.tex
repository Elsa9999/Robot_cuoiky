...
\begin{abstract}
Bài báo trình bày về việc xây dựng môi trường học tăng cường cho robot UR5e thực hiện tác vụ gắp thả vật thể trong mô phỏng vật lý PyBullet. Tác vụ được mô hình hóa xấp xỉ theo quy trình quyết định Markov với không gian quan sát 20 chiều và không gian hành động liên tục 7 chiều. Để giải quyết các thách thức về sự thưa thớt của phần thưởng và hiện tượng tối ưu hóa sai lệch, chúng tôi đề xuất cơ chế tay gắp lai (Hybrid Gripper) tự động kích hoạt dựa trên khoảng cách vật lý, kết hợp bộ ràng buộc tư thế đầu gắp và hàm phần thưởng phân chia theo pha cụ thể. Thuật toán Soft Actor Critic (SAC) được áp dụng để huấn luyện chính sách điều khiển từ đầu. Kết quả thực nghiệm với cấu hình độ khó trung bình (\texttt{difficulty=1}) và trên một seed duy nhất (seed 42) cho thấy sau 600,000 bước huấn luyện, tác tử không ghi nhận episode thất bại trong các lượt đánh giá định kỳ của log hiện có, di chuyển theo quỹ đạo ngắn và an toàn để hoàn thành tác vụ gắp thả. Các giới hạn về tính tổng quát của chính sách và khoảng cách giữa mô phỏng và thực tế cũng được thảo luận thẳng thắn.
\end{abstract}
...
Thuật toán SAC sử dụng hai mạng critic độc lập để ước lượng giá trị trạng thái hành động $Q_{\theta_1}(s, a)$ và $Q_{\theta_2}(s, a)$ nhằm giảm thiểu sự ước lượng quá cao của giá trị $Q$. Quá trình cập nhật các tham số mạng critic được thực hiện bằng cách tối thiểu hóa sai số Bellman:
\begin{equation}
...
Giác hút chân không cần được giữ hướng vuông góc với mặt bàn để việc tiếp cận và hút vật thể đạt độ tin cậy cao hơn. Để đạt được điều này, chúng tôi áp dụng một bộ ràng buộc tư thế vật lý trực tiếp lên giá trị góc đích trước khi gửi đến bộ giải động học nghịch:
\begin{itemize}
...
Phân tích số liệu từ tệp đánh giá thực tế cho thấy các đặc điểm phát triển chính sách qua một số giai đoạn chính:
\begin{itemize}
