...
\begin{abstract}
Bài báo trình bày chi tiết về thiết kế môi trường học tăng cường cho robot UR5e thực hiện tác vụ điều hướng quỹ đạo Cartesian phục vụ gắp và thả vật thể trong mô phỏng vật lý PyBullet. Tác vụ được mô hình hóa xấp xỉ theo quy trình quyết định Markov với không gian quan sát 20 chiều và không gian hành động liên tục 7 chiều, trong đó 6 chiều điều khiển Cartesian thực sự tác động lên môi trường và 1 chiều dành cho tay gắp được thay thế bằng cơ chế gắp thả tự động dựa trên điều kiện hình học. Thuật toán Soft Actor Critic được lựa chọn để huấn luyện chính sách điều hướng cho robot từ đầu mà không cần qua giai đoạn tiền huấn luyện. Nhằm giải quyết các thách thức về sự thưa thớt của phần thưởng và hiện tượng tối ưu hóa sai lệch, chúng tôi đề xuất cơ chế tay gắp lai tự động kích hoạt dựa trên khoảng cách vật lý, bộ ràng buộc tư thế đầu hút và hàm phần thưởng phân chia theo pha cụ thể. Hệ thống được huấn luyện trong 10 triệu bước thời gian với cấu trúc vector hóa song song 16 môi trường và áp dụng chuẩn hóa VecNormalize. Kết quả đánh giá với 20 episode mỗi lần cho thấy từ mốc 600,000 bước, mô hình không ghi nhận episode thất bại, với điểm phần thưởng gốc trung bình đạt 570.20 và số bước thực thi trung bình là 28.65 bước. Kết quả này được ghi nhận trên một seed huấn luyện duy nhất (seed 42) và chỉ trên phân phối vật thể cấu hình \texttt{difficulty=1}. Các kết quả cho thấy tính khả thi của phương pháp thiết kế môi trường và hàm thưởng trong việc định hướng học tập quỹ đạo cho tác tử robot liên tục, đồng thời bài báo thảo luận về các giới hạn hiện tại liên quan đến tính tổng quát của kết quả và khoảng cách giữa mô phỏng và thực tế.
\end{abstract}
...
\begin{itemize}
    ...
    \item Thành phần thứ bảy $a_{gripper}$ được khai báo trong giao diện Gymnasium nhưng không tác động lên môi trường trong thiết kế hiện tại. Logic đóng mở tay gắp được thay thế bằng cơ chế tay gắp lai tự động (Mục IV.A). Chiều hành động này được giữ nguyên trong vector 7 chiều để đảm bảo tương thích với giao diện chuẩn của thư viện Stable Baselines3 và tạo thuận lợi cho các thí nghiệm mở rộng trong tương lai khi cần chuyển sang chế độ tay gắp do tác tử điều khiển. Như vậy, bậc tự do thực sự mà chính sách SAC tác động lên hệ thống chỉ là 6 chiều Cartesian.
\end{itemize}
...
Cần chú ý rằng toàn bộ chính sách được huấn luyện trong nghiên cứu này là từ đầu (train from scratch) trên tệp \texttt{train\_17d\_place.py} trong tổng số 10 triệu bước mẫu với một seed ngẫu nhiên duy nhất (seed 42). Tệp log đánh giá hiện có ghi nhận kết quả đến mốc 6.6 triệu bước. Các điểm phần thưởng báo cáo trong phần kết quả là giá trị gốc trước khi qua bộ lọc VecNormalize. Chúng tôi không sử dụng các kỹ thuật huấn luyện chuyển tiếp hay nạp trước trọng số từ các giai đoạn huấn luyện gắp trước đó, cho thấy hàm phần thưởng thiết kế hỗ trợ tốt để tác tử tự tìm ra chuỗi hành vi điều hướng từ đầu.
...
\begin{itemize}
    ...
    \item \textit{Chiều hành động dư thừa}: Vector hành động khai báo 7 chiều nhưng chiều thứ 7 ($a_{gripper}$) không tác động lên môi trường do cơ chế tay gắp lai thay thế. Mặc dù điều này không gây lỗi về mặt thuật toán, mạng neural của tác tử vẫn phải phân bổ tài nguyên biểu diễn cho một đầu ra không có tín hiệu gradient hữu ích, có thể ảnh hưởng nhẹ đến hiệu quả học.
